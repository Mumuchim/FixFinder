...
// Function to check if a pin is inside a zone
function isInsideZone(pin, zone) {
    const pinRect = pin.getBoundingClientRect();
    const zoneRect = zone.getBoundingClientRect();
...
// Highlight the current zone when a pin is being moved
function highlightZone(pin) {
    const zones = document.querySelectorAll('.confirm-zone');
    zones.forEach(zone => {
        if (isInsideZone(pin, zone)) {
...
// Function to save pin positions to localStorage
function savePinPositions() {
    localStorage.setItem("pinPositions", JSON.stringify(pinPositions));
}

// Load pin positions from localStorage
function loadPinPositions() {
    const savedPositions = JSON.parse(localStorage.getItem("pinPositions") || "[]");
    savedPositions.forEach(position => {
        const pinElement = document.createElement('div');
...
// Initialize pins on window load
window.onload = function () {
    loadPinPositions();
};
